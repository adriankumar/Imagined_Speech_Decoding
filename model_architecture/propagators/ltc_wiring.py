...
#synapse wiring for ltc cell (builds empty adjacency matrix to store connections)
class NeuronSynapseWiring:
    def __init__(self, total_internal_neurons):
        self.total_internal_neurons = total_internal_neurons
        self.neuron_adjacency_matrix = np.zeros([self.total_internal_neurons, self.total_internal_neurons], dtype=np.int8) #dtype for efficiency as the matrix only holds -1, 0 or 1 to indicate polarity of connection 

        self.input_neuron_adjacency = None #will be initialised, stores projection from input to internal neurons 
        self.input_dim, self.final_dim = None, None #final and output dim are equivalent here, but we don't use ltc output layer to project, we are using entire hidden state as features
    ...
    def _validate_neuron_connection(self, src, dst, polarity, reference_size, type="neurons"):
        ...
        if src < 0 or src >= reference_size: #reference size can be total_internal_neurons for add_synapse or input_dim for add_sensory_synapse
...
class NeuralCircuitPolicy(NeuronSynapseWiring):
    def __init__(self, R1_count, R2_count, R3_count,
        ...
        super().__init__(total_internal_neurons=neuron_total) #initialise synapse wiring class to create adjacency matrix for internal neurons
    ...
    def _build_recurrent_connections(self):
        all_neurons = [neuron for neuron in range(0, self.total_internal_neurons)] #indices from all internal neurons
    ...
    def _generate_neuron_indicies(self):
        ...
        self.R1_neurons = [i for i in range((self.R3_neuron_count + self.R2_neuron_count), self.total_internal_neurons)]
