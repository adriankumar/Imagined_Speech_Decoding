from model_architecture import SincNet, ConvHead, extract_band_power, extract_spectrogram
...
spectro_convs = [(24, 8, 5), (16, 5, 3), (12, 4, 2), (8, 3, 1)]
...
class FeatureExtractorv1(nn.Module):
    ...
    def _build_extractors(self):
        ...
        self.sinc_net = SincNet(
            num_sinc_filters=self.sinc_filters, output_dim=self.sinc_out_dim, eeg_channels=self.eeg_channels, 
            segment_length=self.segment_length, freq=self.sfreq, activation=self.sinc_actv
        )

        #takes b x c x seg as input
        self.raw_spatial = ConvHead(
