...
def run_full_inference(model, eeg_sample, segment_length=500):
    ...
    with torch.no_grad():
        ...
        final_signals, cognitive_state= model.think(final_state, cognitive_state, prev_output=prev_output)
...
def compute_gradient_norms(model, eeg_sample, true_words, sentence_encoder, 
    ...
    #final reasoning
    final_state = model.get_final_state()
    final_signals, cognitive_state = model.think(final_state, cognitive_state)
    final_mu, final_log_sigma, policy_state = model.propagate_action(final_signals, policy_state)
