...
class TrainingMetricsTracker:
    ...
    def store_losses(self, world, q1, q2, policy, alpha):
...
def update_actor(model, batcher, q1, q2, alpha, target_entropy, optim, grad_clip, params, q_clamps=[-1, 6]):
    q1.eval()
    q2.eval()
    ...
    alpha_loss = (alpha * (-avg_log_probs - target_entropy)).mean()
    alpha_loss.backward()
    optim[2].step()
    #clamp alpha
    with torch.no_grad():
        alpha.data.clamp(min=1e-4, max=1.0)

    return loss_avg.item(), alpha.item()
...
def training_V4(model, n_epochs, n_eps, segment_length,
                ...
                critic_grad_clip=7.0, actor_grad_clip=7.0, base_alpha=-2.0, alpha_lr=1e-3, world_grad_clip=7.0,
                world_temp=1.0, save_freq=500, world_model_threshold=0.70, fe_threshold=0.60, q_clamps=[-1, 6],
    ...
    #learnable alpha 
    alpha = nn.Parameter(torch.tensor(base_alpha, device=device))
    alpha_optim = torch.optim.Adam([alpha], lr=alpha_lr)
    ...
    for epoch in range(n_epochs):
        ...
        for _ in update_bar:
            ...
            policy_loss, alpha_loss = update_actor(
                model, batcher, Q1, Q2, alpha, target_entropy,
                [fe_optim, policy_optim, alpha_optim], actor_grad_clip, [fe_params, p_params], q_clamps
                )
        ...
        metrics_tracker.store_losses(world_model_loss, Q1_loss, Q2_loss, policy_loss, alpha_loss)
        #save epoch metrics
        ...
        if stats['avg_reward'] > fe_threshold and not fe_train:
            for params in model.feature_extractor.parameters():
                params.requires_grad = True
            fe_train = True
            print("Unfroze feature extractor parameters")

        if stats['avg_reward'] > world_model_threshold and not enable_world_training: #only train world model when reward increases
            enable_world_training = True
            print("Enabled world training")

        print(f"avg reward: {stats['avg_reward']:.4f} | avg policy loss: {stats['avg_policy_loss']:.4f} | avg world loss: {stats['avg_world_loss']:.4f} | avg Q1 loss: {stats['avg_q1_loss']:.4f} | avg Q2 loss: {stats['avg_q2_loss']:.4f} | avg alpha loss: {stats['avg_alpha_loss']:.4f}")
