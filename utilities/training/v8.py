#give cognitive layer lower/slower learning rate so policy components actually update
...
from model_architecture import QNetworkv2_1 as QNetwork
...
class TrainingMetricsTracker:
    ...
    def store_losses(self, q1, q2, policy):
        self.Q1_losses.append(q1)
        self.Q2_losses.append(q2)
        self.policy_losses.append(policy)

    def compute_epoch_stats(self, epoch_num, n_episodes_this_epoch, update_steps):
        ...
        #loss statistics
        self.mean_q1_loss.append(np.mean(self.Q1_losses[-update_steps:]))
        self.mean_q2_loss.append(np.mean(self.Q2_losses[-update_steps:]))
        self.mean_policy_loss.append(np.mean(self.policy_losses[-update_steps:]))
...
def compute_timestep_reward(model, action, true_words, epoch, is_final=False):

    threshold = min(0.65 + (epoch / 500) * 0.5, 0.85) #dynamic threshold for reward signals
    ...
    return compute_rewards(model, action, true_words, weights, threshold)

def compute_rewards(model, action, true_words, weights, threshold=0.15):
...
#soft version of semantic coverage
def compute_semantic_coverage_soft(pred_embeds, true_embeds):
    ...
    # Soft coverage: use actual max similarities (scaled to [0, 1])
    max_sim_per_word = sim.max(dim=1)[0]  # n_content
    coverage = max_sim_per_word.mean()  # Average similarity
    
    # Soft efficiency: penalize using too many steps
    steps_contribution = sim.max(dim=0)[0]  # Max similarity per step
    steps_used = (steps_contribution > 0.1).sum()  # Count "active" steps
    efficiency = 1.0 - (steps_used / thought_steps)
...
def episodes_rollout(model, train_tensor, id_tensor, label_mapping, n_eps, seg_len, current_epoch,
    ...
    with torch.no_grad():
        for ep in episodes_bar:
            ...
            for i, window in enumerate(windows):
                ...
                reward_t = compute_timestep_reward(model, sampled_action, true_words, current_epoch, is_final=False)
            ...
            final_reward = compute_timestep_reward(model, final_act, true_words, current_epoch, is_final=True)
...
def init_optim_and_schedulers(q1, q2, lr_q, model, lr_policy, adaptive_lr,
                              q_mode, q_factor, q_patience, q_min_lr, 
                              policy_mode, policy_factor, policy_patience, policy_min_lr
                              ):

    # Init Q-network optimizers
    q1_optim = torch.optim.Adam(q1.parameters(), lr=lr_q)
    q2_optim = torch.optim.Adam(q2.parameters(), lr=lr_q)

    policy_params = [
        # Feature extractor: frozen until threshold >= 0.2, then slow fine-tuning
        {'params': model.feature_extractor.parameters(), 'lr': lr_policy * 0.001},
        
        # Cognitive layer: not pretrained, needs to learn how to "think"
        {'params': model.cognitive_layer.parameters(), 'lr': lr_policy * 0.001},
        
        # Policy propagator: moderate speed (transforms cognitive → motor)
        {'params': model.policy_prop.parameters(), 'lr': lr_policy * 0.001},
        
        # Mean projector: VERY SLOW (preserve pretrained semantic embeddings!)
        {'params': model.mean_projector.parameters(), 'lr': lr_policy * 0.001}, 
        
        # Variance projector: moderate-fast (needs to find optimal exploration)
        {'params': model.variance_projector.parameters(), 'lr': lr_policy * 0.01},  
    ]


    policy_optim = torch.optim.Adam(policy_params)

    # Flatten params for gradient clipping (needed in update_actor)
    flat_params = []
    for group in policy_params:
    ...
    if adaptive_lr:
        scheduler_q1 = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer=q1_optim, mode=q_mode, factor=q_factor, 
            patience=q_patience, min_lr=q_min_lr
        )

        scheduler_q2 = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer=q2_optim, mode=q_mode, factor=q_factor, 
            patience=q_patience, min_lr=q_min_lr
        )

        scheduler_policy = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer=policy_optim, mode=policy_mode, factor=policy_factor, 
            patience=policy_patience, min_lr=policy_min_lr
        )
    
    schedulers = [scheduler_q1, scheduler_q2, scheduler_policy]
    
    # Return flat_params instead of policy_params for gradient clipping
    return flat_params, q1_optim, q2_optim, policy_optim, schedulers
...
def compute_bellman_targets(model, batcher, TQ1, TQ2, gamma, alpha, q_clamps=[-1, 6]):
    ...
    targets = []
    TQ1_l_state, TQ1_a_state, tq1_prev = None, None, None 
    TQ2_l_state, TQ2_a_state, tq2_prev = None, None, None
    ...
    for i in range(batcher.n_windows):
        ...
        with torch.no_grad():
            ...
            tq1_pred, TQ1_l_state, TQ1_a_state = TQ1(next_state, next_action, TQ1_l_state, TQ1_a_state, tq1_prev)
            tq2_pred, TQ2_l_state, TQ2_a_state = TQ2(next_state, next_action, TQ2_l_state, TQ2_a_state, tq2_prev)
...
def get_critic_losses(batcher, q, targets, q_clamps):
    ...
    q_l_state, q_a_state, q_prev = None, None, None 
    ...
    for i in range(batcher.n_windows):
        ...
        #predict q values 
        q_pred, q_l_state, q_a_state = q(current_state, current_action, q_l_state, q_a_state, q_prev)

        q_prev = q_pred #assign before clamping

        #clamp q pred - not clamping only for training so if q networks predict outside of expected range
        #gradients will still contribute in the learning, because if its clipped then 0 gradients for learning
        #only keeping clamp for bellman target computations and maybe update actor?
...
def update_actor(model, batcher, q1, q2, 
                 log_alpha, log_alpha_optim, target_entropy, learnable_alpha,
                 optim, grad_clip, params, q_clamps=[-1, 6]):
    q1.eval()
    q2.eval()

    q1_l, q1_a, q1_prev = None, None, None 
    q2_l, q2_a, q2_prev = None, None, None
    cognitive_state, motor_state, prev_output = None, None, None 

    policy_losses = []
    log_probs = []

    optim.zero_grad() #feature extractor optim

    alpha_for_model = get_alpha(True, True, log_alpha) #keep true so that even for non learnable case we retrieve the exp() cos we use raw log alpha for loss, which is counter intuitive i need to fix the logic

    # alpha = log_alpha.exp().detach() #get current alpha value (detach for policy loss, we don't want policy gradients flowing to alpha)
    ...
    for i in range(batcher.n_windows):
        ...
        with torch.no_grad():
            ...
            q1_pred, q1_l, q1_a = q1(state_t.unsqueeze(-1), action_t, q1_l, q1_a, q1_prev)
            q2_pred, q2_l, q2_a = q2(state_t.unsqueeze(-1), action_t, q2_l, q2_a, q2_prev)

            #assign raw prev value, not clampped
            q1_prev = q1_pred 
            q2_prev = q2_pred
        ...
        policy_loss = compute_policy_loss(q1_pred, q2_pred, log_prob_t, alpha_for_model)
        policy_losses.append(policy_loss)

    #compute policy update first
    loss_avg = torch.stack(policy_losses, dim=0).mean()
    loss_avg.backward()
    # variance_params = list(model.variance_projector.parameters())
    # torch.nn.utils.clip_grad_norm_(variance_params, max_norm=0.5)  #clip variance separately
    # non_variance_params = [p for p in params if p not in variance_params]
    torch.nn.utils.clip_grad_norm_(params, max_norm=grad_clip) #clip policy params separate from variance
    ...
    if learnable_alpha:
        ...
        alpha_loss = -(log_alpha * (avg_log_probs + target_entropy)).mean() #use log alpha directly
        alpha_loss.backward()
        torch.nn.utils.clip_grad_norm_([log_alpha], max_norm=1000.0)
    ...
    return loss_avg.item(), alpha_loss, avg_log_probs.item()
...
def dynamic_update_steps(epoch, avg_reward=None, min_steps=10, max_steps=40, warmup_epochs=50):
...
def get_alpha(learnable, detach, base_value):
    if learnable:
        if detach:
            return base_value.exp().detach()
        return base_value.exp()
    else:
        # Fixed alpha is also stored as log, need to exponentiate
        return base_value.exp() if isinstance(base_value, torch.Tensor) else base_value

#just to re-init after loading pretrained model before sac training
def reinit_variance(model, gain=0.01, bias_init=-2.0):
    nn.init.xavier_uniform_(model.variance_projector.weight, gain=gain)
    nn.init.constant_(model.variance_projector.bias, bias_init)

#uses qnetwork v2_1 and learnable alpha because fixed state t input
def training_V8(model, n_epochs, n_eps, segment_length,
                ...
                critic_grad_clip=7.0, actor_grad_clip=7.0, learnable_alpha=True,
    ...
    p_params, q1_optim, q2_optim, policy_optim, schedulers = init_optim_and_schedulers(
        Q1, Q2, lr_q, model, lr_policy, adaptive_lr,
        qmode, qfac, qpat, qmin_lr,
        pmode, pfac, ppat, pmin_lr, 
    )
    ...
    #learnable alpha
    if learnable_alpha:
        log_alpha = nn.Parameter(torch.tensor(np.log(0.0008326), device=device))
        log_alpha_optim = torch.optim.Adam([log_alpha], lr=5e-5)
        target_entropy = -(model.embedding_dim * model.thought_steps)
    else:
        log_alpha = torch.tensor(np.log(0.0008326), device=device)
        log_alpha_optim = None 
        target_entropy = 0.0
    
    print(f"using learnable alpha: {learnable_alpha}")
    ...
    display_value = 0 #for epoch monitoring of log probs if using learnable alpha or not
    prev_value = 0
    display_str = "entropy gap" if learnable_alpha else "log prob diff"
    ...
    for epoch in range(n_epochs):
        ...
        experience_replay = episodes_rollout(
            model, train_tensor, label_id_tensor, label_mapping, n_eps, segment_length, epoch,
        ...
        for _ in update_bar:
            ...
            #compute target q values  - log_alpha.exp() makes it alpha
            alpha_bellman = get_alpha(learnable_alpha, True, log_alpha)
            q_targets = compute_bellman_targets(model, batcher, TQ1, TQ2, gamma, alpha_bellman, q_clamps)
            ...
            #update policy 
            policy_loss, alpha_loss, avg_lp = update_actor(
                model, batcher, Q1, Q2, log_alpha, log_alpha_optim, target_entropy, learnable_alpha,
                policy_optim, actor_grad_clip, p_params, q_clamps
                )
            
            avg_alpha_loss += alpha_loss
            ...
            #store metrics
            metrics_tracker.store_losses(Q1_loss, Q2_loss, policy_loss)

        #save epoch metrics
        stats = metrics_tracker.compute_epoch_stats(epoch+1, n_eps, update_steps)
        
        avg_alpha_loss = avg_alpha_loss / update_steps
        ...
        threshold = min(0.65 + (epoch / 500) * 0.5, 0.85)

        #unfreeze feature extractors for fine-tuning
        if threshold >= 0.7 and not fe_unfrozen:
            for param in model.feature_extractor.parameters():
                param.requires_grad = True
            
            fe_unfrozen = True
            print("Feature extractors UNFROZEN for fine-tuning")

        if learnable_alpha:
            display_value = avg_lp + target_entropy #monitor entropy gap if learnable alpha
        else:
            display_value = np.abs(avg_lp - prev_value) #otherwise just monitor the log prob difference at each epoch
            prev_value = avg_lp

        print(f'avg reward: {stats["avg_reward"]:.6f} | reward threshold: {threshold:.4f} | avg policy loss: {stats["avg_policy_loss"]:.6f} |'
              f'avg_q1_loss: {stats["avg_q1_loss"]:.6f} | avg_q2_loss: {stats["avg_q2_loss"]:.6f} |'
              f'avg_a_loss: {avg_alpha_loss:.6f} | current alpha: {alpha_bellman:.8f} |'
              f'avg_log_probs: {avg_lp:.0f} | {display_str}: {display_value:.0f}')
