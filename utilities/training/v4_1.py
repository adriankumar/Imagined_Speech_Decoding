    ...
    with torch.no_grad():
        for ep in episodes_bar:
            ...
            for i, window in enumerate(windows):
                ...
                sampled_action, signals, cognitive_state, policy_state = experience_forward_pass(
...
def compute_bellman_targets(model, batcher, TQ1, TQ2, gamma, log_alpha, q_clamps=[-1, 6]):
    ...
    for i in range(batcher.n_windows):
        ...
        with torch.no_grad():
            ...
            target = reward_t + gamma * (1.0 - done) * (min_q - log_alpha.exp() * next_log_prob) #batch x 1
...
def update_actor(model, batcher, q1, q2, log_alpha, target_entropy, optim, grad_clip, params, q_clamps=[-1, 6]):
    q1.eval()
    q2.eval()
    ...
    for i in range(batcher.n_windows):
        ...
        policy_loss = compute_policy_loss(q1_pred, q2_pred, log_prob_t, log_alpha.exp())
    ...
    return loss_avg.item(), log_alpha.exp().item()
...
def training_V4_1(model, n_epochs, n_eps, segment_length,
                ...
                save_freq=500, fe_threshold=0.60, q_clamps=[-1, 6],
    ...
    for epoch in range(n_epochs):
        ...
        for _ in update_bar:
            ...
            #compute target q values 
            q_targets = compute_bellman_targets(model, batcher, TQ1, TQ2, gamma, log_alpha, q_clamps)
            ...
            policy_loss, alpha_loss = update_actor(
                model, batcher, Q1, Q2, log_alpha, target_entropy,
                [fe_optim, policy_optim, log_alpha_optim], actor_grad_clip, [fe_params, p_params], q_clamps
                )
