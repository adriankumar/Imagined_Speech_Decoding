//canvas drawing helpers: crisp setup, diverging sign colour, and the harmonic system Y^T c = b over all features
...
//diverging map: pale toward blue for positive, pale toward red for negative, scaled by the field's absmax
function signColor(v, absmax){
  const c = Math.max(-1, Math.min(1, absmax > 0 ? v / absmax : 0));
  if(c >= 0) return [0.95 - 0.78 * c, 0.95 - 0.55 * c, 0.95];
  const a = -c;
  return [0.95, 0.95 - 0.55 * a, 0.95 - 0.78 * a];
}
...
function drawHarmonicSystem(g, w, h, data){
  ...
  const ch  = Math.max(1, Math.min(7, (h - padT - padB) / n));
  const cwY = Math.max(2, Math.min(14, (w * 0.40) / M));            //Y^T columns, the modes, wider cap
  const cwF = Math.max(12, Math.min(40, (w * 0.20) / Math.max(F, 1)));//c and b feature columns, wider cap
  ...
  //c, M rows by F cols, sits at the top aligned with Y^T's mode columns
  ...
  g.fillText(`Y^T  n=${n} x M=${M}`, yX + matWY / 2, yTop - 8);
  g.fillText(`c  M=${M} x F=${F}`, cX + matWF / 2, yTop - 8);
  g.fillText(`b  n=${n} x F=${F}`, bX + matWF / 2, yTop - 8);

  //operator glyphs between the blocks
  g.fillStyle = "#9a978f"; g.font = "14px sans-serif"; g.textBaseline = "middle";
  g.fillText("\u00d7", cX - gap / 2, yTop + (M * ch) / 2);
  g.fillText("=", bX - gap / 2, yTop + (n * ch) / 2);

  //per-feature compression residual under the b block
  if(residual){
    g.fillStyle = "#6b6a66"; g.font = "9px sans-serif"; g.textAlign = "center"; g.textBaseline = "top";
    const ry = yTop + n * ch + 6;
    for(let j = 0; j < F; j++){
      g.fillText(names[j], bX + j * cwF + cwF / 2, ry);
      g.fillText("r=" + residual[j].toFixed(2), bX + j * cwF + cwF / 2, ry + 11);
    }
  }
}
...
function drawCoeffMatrix(g, w, h, data){
  ...
  if(title) g.fillText(title, x0, 16);
}